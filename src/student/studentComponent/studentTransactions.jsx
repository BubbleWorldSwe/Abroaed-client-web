            ...
            <div className="bg-white p-6 mb-5 shadow rounded-lg">
                ...
                <div className="bg-[#FAFAFA] px-10 py-5">
                    ...
                    <div className="mt-1 flex justify-between items-center">
                        <div>
                            <p className="text-sm text-gray-600 ">Company Name: <span className="font-medium">Viking Burrito</span></p>
                            <p className="text-sm text-gray-600">Email Address: <span className="font-medium">oliver@burrito.com</span></p>
                            <p className="text-sm text-gray-600">VAT Number: <span className="font-medium">FRB1235476</span></p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <tbody>
                            {transactionsDetails.map((tx, index) => (
                                <tr key={index} className="text-gray-700 border-b">
                                    ...
                                    <td className="p-3 flex items-center gap-2">{tx.amount} {index === 0 && <span className="text-red-500">•</span>}</td>
                                    <td className="p-3">{tx.description}</td>
                                    <td className="p-3 flex justify-between items-center ">
                                        {tx.mode}
                                        {tx.invoice && <button className="px-3 py-1 text-sm border bg-[#F4F4F5] rounded-lg text-gray-700 hover:bg-gray-200">View Invoice</button>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
