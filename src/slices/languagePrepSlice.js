...
export const fetchColleges = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        return new Promise((resolve) => {
            setTimeout(() => {
                resolve([...LangaugePrep]); // Return the updated mock array.
            }, 1000);
        });
    }
);
...
export const addCollege = createAsyncThunk(
    ...
    async (collegeData, { dispatch, rejectWithValue }) => {
        return new Promise((resolve) => {
            setTimeout(() => {
                LangaugePrep.push(collegeData); // Append the new college to the mock array.
...
const languagePreSlice = createSlice({
    name: "colleges",
    initialState: {
        languages: [],
        loading: false,
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchColleges.fulfilled, (state, action) => {
                state.loading = false;
                state.languages = action.payload; // Update the colleges state with the new list.
            })
...
export default languagePreSlice.reducer;
